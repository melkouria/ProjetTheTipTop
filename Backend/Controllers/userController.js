        ...
        try{
            const user = await Users.findOne({email:req.body.email});
                if(user.isVerified){
                    ...
                    console.log('Sil vous plaiz verifier votre compte email')
                    res.json({status :   'Sil vous plaiz verifier votre compte email'})
                }
    ...
    await Users.create(result).then(userStoreData =>{
      ...
      var mailOptions={
        ...
        html:`<h2> ${result.nom}! Merci de vous etre inscrit sur notre site </h2>
             ...
             <a href="http://104.197.137.251:7777/verify-email?token=${result.emailToken}"> Verifier Votre Email </a> `
     }
...
var transporter = nodemailer.createTransport({
    service: 'gmail',
    auth:{
        user:'mouradamehiou20@gmail.com',
        pass:'reeyhijbraliiaxm'
    },
    tls:{
